	...
	void collisionX() {
		...
		for (int i = rect.top / 30; i < (rect.top + rect.height) / 30; i++)
			for (int j = rect.left / 30; j < (rect.left + rect.width) / 30; j++) {
				...
				if ((tileMap[i][j] == '1') && over_game == false) {
					key1 = true;
					win_l1 = true;
					game = true;
				}
...
public:
	...
	drzewo(Texture* texture, Vector2u windowSize)
	{
		this->shape.setTexture(*texture);
		this->shape.setPosition(550, (rand() % 350 + 300));
	}
};
...
public:
	...
	drzewog(Texture* texture, Vector2u windowSize)
	{
		this->shape.setTexture(*texture);
		this->shape.setPosition(550, (rand() % (1) + 0.1));
	}
};
...
public:
	...
	int score;
	int scoreMax;
	orzeszek(Texture* texture, Vector2u windowSize)
	{
		this->scoreMax = rand() % 3 + 1;
...
int main()
{
	...
	wiewiureczka:
		...
		RenderWindow windowe(VideoMode(512, 512), "game+", Style::Titlebar | Style::Close);
		Event ev;
		...
		Texture playerTex;
		playerTex.loadFromFile("wiewiora.png");
		Texture drzewoTex;
		drzewoTex.loadFromFile("drzewodol.png");
		Texture drzewogTex;
		drzewogTex.loadFromFile("drzewogora.png");
		Texture orzeszekTex;
		orzeszekTex.loadFromFile("orzeszek.png");
		...
		float drzewoSpawnTimer = 0;
		vector<drzewo> drzewa;

		float drzewogSpawnTimer = 0;
		vector<drzewog> drzewag;


		float orzeszekSpawnTimer = 0;
		vector<orzeszek> orzeszki;
		Text oHpText;
		oHpText.setFont(font);
		oHpText.setCharacterSize(12);
		oHpText.setFillColor(Color::Black);
		...
		while (windowe.isOpen())
		{
			...
			dt = clockkk.restart().asSeconds();
			if (wiewiurka.HP > 0) {

				if (Keyboard::isKeyPressed(Keyboard::Key::Up)) {
					wiewiurka.shape.move(0.f, -10.f * dt * dtMultiplier);
				}
				else {

					wiewiurka.shape.move(0.f, 4.f * dt * dtMultiplier);
				}

				hpText.setPosition(wiewiurka.shape.getPosition().x, wiewiurka.shape.getPosition().y - hpText.getGlobalBounds().height);
				hpText.setString(to_string(wiewiurka.HP) + "/" + to_string(wiewiurka.HPMax));





				if (wiewiurka.shape.getPosition().y < 0.f) {

					wiewiurka.shape.setPosition(wiewiurka.shape.getPosition().x, 0.f);
				}
				if (wiewiurka.shape.getPosition().y + wiewiurka.shape.getGlobalBounds().width > 512) {

					wiewiurka.shape.setPosition(wiewiurka.shape.getPosition().x, 512 - wiewiurka.shape.getGlobalBounds().height);
				}

				if (drzewoSpawnTimer < 50) {

					drzewoSpawnTimer += 1.f * dt * dtMultiplier;
				}
				if (drzewoSpawnTimer >= 50) {

					drzewa.push_back(drzewo(&drzewoTex, windowe.getSize()));
					drzewoSpawnTimer = 0;
				}
				for (size_t i = 0; i < drzewa.size(); i++) {

					drzewa[i].shape.move(-6.f * dt * dtMultiplier, 0.f);

					if (drzewa[i].shape.getPosition().x <= 0 - drzewa[i].shape.getGlobalBounds().width) {

						drzewa.erase(drzewa.begin() + i);
						break;
					}

					if (drzewa[i].shape.getGlobalBounds().intersects(wiewiurka.shape.getGlobalBounds())) {
				...
				if (drzewogSpawnTimer < 50) {

					drzewogSpawnTimer += 1.f * dt * dtMultiplier;
				}
				if (drzewogSpawnTimer >= 50) {

					drzewag.push_back(drzewog(&drzewogTex, windowe.getSize()));
					drzewogSpawnTimer = 0;
				}
				for (size_t i = 0; i < drzewag.size(); i++) {

					drzewag[i].shape.move(-6.f * dt * dtMultiplier, 0.f);

					if (drzewag[i].shape.getPosition().x <= 0 - drzewag[i].shape.getGlobalBounds().width) {

						drzewag.erase(drzewag.begin() + i);
						break;
					}

					if (drzewag[i].shape.getGlobalBounds().intersects(wiewiurka.shape.getGlobalBounds())) {
				...
				if (orzeszekSpawnTimer < 50) {

					orzeszekSpawnTimer += 1.f * dt * dtMultiplier;
				}
				if (orzeszekSpawnTimer >= 50) {

					orzeszki.push_back(orzeszek(&orzeszekTex, windowe.getSize()));
					orzeszekSpawnTimer = 0;
				}
				for (size_t i = 0; i < orzeszki.size(); i++) {

					orzeszki[i].shape.move(-6.f * dt * dtMultiplier, 0.f);

					if (orzeszki[i].shape.getPosition().x <= 0 - orzeszki[i].shape.getGlobalBounds().width) {

						orzeszki.erase(orzeszki.begin() + i);
						break;
					}

					if (wiewiurka.shape.getGlobalBounds().intersects(orzeszki[i].shape.getGlobalBounds())) {
						if (orzeszki[i].score <= 3) {

							score += orzeszki[i].scoreMax;
							orzeszki.erase(orzeszki.begin() + i);
						}
					}
				}
			...
			windowe.draw(wiewiurka.shape);

			for (size_t i = 0; i < drzewa.size(); i++) {

				windowe.draw(drzewa[i].shape);
			}

			for (size_t i = 0; i < drzewag.size(); i++) {

				windowe.draw(drzewag[i].shape);
			}

			for (size_t i = 0; i < orzeszki.size(); i++) {

				oHpText.setString(to_string(orzeszki[i].score));
				oHpText.setPosition(orzeszki[i].shape.getPosition().x, orzeszki[i].shape.getPosition().y - oHpText.getGlobalBounds().height);
				windowe.draw(oHpText);
				windowe.draw(orzeszki[i].shape);
			}
			if (score >= 10) {
...
gra:
	...
	while (window.isOpen())
	{
		...
		clock.restart();
		time = time / 800;// spowolnienie czasu aby ruch i animacja byly lepsze
		...
		window.display();
		//cout << "l=   " << player.rect.left << "   h=  " << player.rect.top << endl;
		x = player.rect.left;
		y = player.rect.top;
	}
